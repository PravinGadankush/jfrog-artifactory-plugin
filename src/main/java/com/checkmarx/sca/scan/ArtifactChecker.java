...
public class ArtifactChecker {
    ...
    public void checkArtifact(@Nonnull RepoPath repoPath) {
        ...
        if (repoConfiguration instanceof VirtualRepositoryConfiguration) {
            setPhysicalRepoPathsOfVirtualRepository(physicalRepoPaths, repoConfiguration, repoPath.getName());
        } else {
        ...
        try {
            ...
            artifactId = _artifactIdBuilder.getArtifactId(fileLayoutInfo, packageType);

            if (artifactId.isInvalid()) {
                _logger.error(format("The artifact id was not built correctly. RepoPath name: %s",repoPath.getName()));
                return;
            }
        } catch (Exception ex) {
    ...
    private void setPhysicalRepoPathsOfVirtualRepository(ArrayList<RepoPath> physicalRepoPaths, RepositoryConfiguration repoConfiguration, String repoPathName){
        ...
        for (var repo : virtualConfiguration.getRepositories()) {
            var repoPathFromVirtual = RepoPathFactory.create(repo, repoPathName);
